...
| `tests/` | 24 файла тестов, без сети |
...
## Размер файлов (`[DEV-FILE-SIZE]`)

Жёсткий предел для `.py` в `app/`, `companion/`, `tests/` — 25 000 байт
или 800 строк, цель — до 20 000 байт и 500 строк; проверяет
`tests/test_file_size.py`. Новый функционал идёт в отдельный модуль,
а не в ближайший большой файл.

Унаследованные крупные модули в списке `LEGACY` того же теста:
`mail.py` (~43 КБ), `format.py` (~38 КБ), `vision.py` (~37 КБ),
`refs.py` (~30 КБ). Им разрешено только уменьшаться; при правке
читается нужная функция, а не весь файл. Образец разбиения с
совместимыми именами — `tests/test_split_compat.py`.

...
Скрипт трогает только части обновления: ставит unit, даёт право запуска
`ota-update.sh`, кладёт правило sudo, создаёт папку состояния и перечитывает
`systemd`. Саму программу, `.env` и данные он не меняет. Правило sudo проверяется
`visudo -c` и при ошибке удаляется, чтобы не сломать sudo на сервере.
...
Сообщение «Служба обновления не установлена» теперь перечисляет, чего именно

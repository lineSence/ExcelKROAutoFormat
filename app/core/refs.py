...
def lookup(
    ...
    if visit_key:
        ...
        if admin or people:
            ...
            if not people:
                notes.append(
                    "Ревизоры в графике не распознаны: в блоке распределения "
                    "фамилий нет. Впишите их вручную. Проверить, что лежит в "
                    "книге, можно командой: python -m tools.refs_doctor "
                    f"--dump --date {day:%d.%m.%Y}"
                )

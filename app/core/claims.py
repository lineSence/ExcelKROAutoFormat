...
* выделяются зелёным по всей ширине строки (A:L);
* получают комментарий в столбце L вида «недовоз ПДВ06030196 3шт.»;
...
# Комментарий пишется в столбец L, заливка идёт по столбцам A:L.
COL_COMMENT = 12
SPAN_COLUMNS = 12
...
def comment(kind: str, docs: tuple[str, ...], qty: float) -> str:
    """Комментарий для столбца L: «недовоз ПДВ06030196 3шт.»."""
    numbers = " ".join(docs)

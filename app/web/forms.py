...
import re
VERIFY_MODES=("off","shadow","on")
DEFAULT_LOGIC=100
META_MULTI_FIELDS=("sellers","sellers_post","checkers","checkers_post")
CLAIM_PREFIX="claim-"
PAIR_PREFIX="pair-"
UNSAFE_NAME=re.compile(r"[^\w.\-]+", re.UNICODE)
def mode(raw: str|None)->str:
    value=str(raw or "").strip().lower(); return value if value in VERIFY_MODES else "off"
def percent(raw: object,fallback:int=DEFAULT_LOGIC)->int:
    text=str(raw if raw is not None else "").strip().replace(",",".")
    if not text: return max(0,min(100,int(fallback)))
    try: value=int(round(float(text)))
    except ValueError: return max(0,min(100,int(fallback)))
...
def whole_number(raw: object,fallback:int=0)->int:
    ...
    try: value=int(float(text.replace(",",".")))
    except ValueError: return fallback
    return value if value>0 else fallback
def safe_name(raw: str|None)->str:
    text=str(raw or "").strip().replace("\\","/"); tail=text.rsplit("/",1)[-1]
    cleaned=UNSAFE_NAME.sub("_",tail).strip("._")

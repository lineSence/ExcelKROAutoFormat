...
def index_page(request: Request, error="", strict=None, verify=None, logic=None, status=200):
    current = base()
    logic = int(round(float(getattr(current, "logic_weight", 1.0)) * 100)) if logic is None else logic
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "strict": settings.strict_resort if strict is None else bool(strict),
            "verify": default_verify if verify is None else mode(verify),
            "default_verify": default_verify,
            "logic": percent(logic),
            "model": model_status(settings),
            "embed": runtime.embed_status(current),
            "judge": runtime.judge_status(current),
            "jobs": job_cards(),
        },
        status_code=status,
    )
...
def error_page(request, message, strict=False, verify="off", logic=DEFAULT_LOGIC, status=400):
    return index_page(request, error=message, strict=strict, verify=verify, logic=logic, status=status)


def result_page(request, job: Job, message=""):
    result = job.result
    pending = [x for x in result.doubtful if not x.get("answered")]
    link = letters_service.letters_for(result, letters)
    note = letters_service.mail_note(link["mine"])
    report = job.mail_vision or {}
    if not message and note["letters"]:
        message = mail_vision.summary(report)
    elif not message and link["others"]:
        message = link["note"] or str(report.get("note") or "")
    return templates.TemplateResponse(
        request=request,
        name="result.html",
        context={
            ...
            "verify": mode(job.verify),
            "logic": percent(job.logic),
            "refs": result.refs,
            "meta": result.sheet_meta.to_form(),
            "comparison": result.comparison,
            "claims": result.claims,
            "message": message,
            "photos": len(job.photos),
            "mail_note": note,
...
def vision_page(request, message="", error="", results=None, token="", status_code=200):
    ...
    return templates.TemplateResponse(
        ...
        name="vision.html",
        context={
            "vision": vision_core.status(current, config),
            "config": config,
...
def mail_page(request, message="", error="", status_code=200, token=""):
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "rows": letters_service.mail_list(letters),
            "jobs": cards,
            "token": only_job(token, cards),
...
def refs_page(request, note="", error=""):
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "auto_confirm": bool(getattr(current, "refs_auto_confirm", True)),
            "note": note,
            "error": error,
        },
    )
...
def update_page(request, message="", error="", status_code=200):
    ...
    return templates.TemplateResponse(
        request=request,
        name="update.html",
        context={"status": info, "state": info["state"], "message": message, "error": error},
        status_code=status_code,
    )
...
def training_page(request, message="", error="", report=None, status_code=200):
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "embed": runtime.embed_status(current),
            "judge": runtime.judge_status(current),
            "stats": dataset_stats(settings.train_store_path),
            "message": message,
            "error": error,
            "report": report,
            "max_upload_mb": settings.max_upload_mb,

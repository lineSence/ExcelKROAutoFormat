...
async def training_samples(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        if not samples:
            return render.training_page(request, error="В файле нет готовых примеров.", status_code=400)
        added = await run_in_threadpool(append_samples, settings.train_store_path, samples)
    ...
    return render.training_page(request, message=f"Добавлено примеров: {added}.")
...
async def training_embed(request: Request, embed: str | None = Form(default=None), provider: str = Form(default=""), api_key: str = Form(default=""), model: str = Form(default=""), timeout: str = Form(default=""), max_requests: str = Form(default="")):
    fields = {"embed_enabled": is_on(embed), "embed_provider": provider.strip(), "embed_model": model.strip(), "embed_timeout": whole_number(timeout, 0), "embed_max_requests": whole_number(max_requests, 0)}
    if api_key.strip(): fields["embed_api_key"] = api_key.strip()
    try:
        await run_in_threadpool(runtime.save_embed, fields, settings)
    except Exception as error:
        logger.exception("Настройки эмбеддингов не сохранены")
        return render.training_page(request, error=settings_not_saved(error, runtime.runtime_path(settings)), status_code=500)
...
async def training_embed_key_clear(request: Request):
    try:
        ...
        return render.training_page(request, error=key_not_cleared("Ключ", error, runtime.runtime_path(settings)), status_code=500)
    return render.training_page(request, message="Ключ удалён.")

@router.post("/training/judge")
async def training_judge(request: Request, provider: str = Form(default=""), api_key: str = Form(default=""), model: str = Form(default=""), api_url: str = Form(default=""), timeout: str = Form(default=""), retries: str = Form(default=""), max_requests: str = Form(default=""), batch: str = Form(default=""), max_pairs: str = Form(default=""), logic: str = Form(default="")):
    fields = {"judge_provider": provider.strip(), "judge_model": model.strip(), "judge_api_url": api_url.strip(), "judge_timeout": whole_number(timeout, 0), "judge_retries": whole_number(retries, 0), "judge_max_requests": whole_number(max_requests, 0), "judge_batch": whole_number(batch, 0), "judge_max_pairs": whole_number(max_pairs, 0), "logic_weight": percent(logic) / 100}
    if api_key.strip(): fields["judge_api_key"] = api_key.strip()
    try:
        await run_in_threadpool(runtime.save_judge, fields, settings)
    except Exception as error:

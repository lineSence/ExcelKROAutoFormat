...
(`mail_vision`), ответы модели попадают на страницу «Фото товара» по тому же
токену, а кнопка архива письма рядом со скачиванием файла отдаёт уже
...
# Отчёт о разборе снимков из писем по токену сверки: разбор идёт один
# раз при обработке, а не при каждом скачивании архива.
...
async def upload(
    ...
    # Снимки из писем разбираются здесь же: сверка уже готова, излишки
    # известны, а архив письма после этого скачивается уже обработанным.
...
async def _rebuild(
    ...
    # Повторно в модель снимки не гоняем: разбор нужен только тогда, когда

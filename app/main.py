...
Письма ревизоров из последнего захода в ящик видны и на странице готовой
сверки: рядом с кнопкой скачивания файла стоит кнопка архива письма, а
подсказки из текста (товар в списке несосчитанного, ФИО ночного продавца)
...
def _name_photos(letter, token: str = "") -> None:
    ...
    Кандидаты берутся из открытой сверки. Нет ключа, выключено
    распознавание или нет сверки — снимки остаются со своими именами:
    архив всё равно должен скачаться.
    """
...
async def mail_archive(uid: str, token: str = ""):
    """Отдаёт архив письма: все вложения и текст письма отдельным файлом.

    Снимки в архиве названы товаром, который узнала нейросеть, если
    распознавание включено и открыта сверка (её номер приходит в `token`).
    Иначе файлы сохраняют свои имена — архив всё равно собирается.
    """

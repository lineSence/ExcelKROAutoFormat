...
class FakeClient(vision.GigaChatVision):
    def __init__(self, answer: str = "", error: str = "") -> None:
        ...
        self.error = error
        self.calls: list[str] = []
    def ask(self, image, mime, prompt):
...
def test_model_answer_is_checked_and_cached(tmp_path):
    ...
    client = FakeClient(answer="```json\n" + answer(text="HUSKY MINT 30ml", candidates=[{"row": 12, "confidence": 0.8}, {"row": 999, "confidence": 0.9}]) + "\n```")
...
def test_text_fallback_when_model_gives_no_row(tmp_path):
    settings = make_settings(tmp_path); config = turn_on(settings); client = FakeClient(answer=answer(text="Жидкость Husky Mint 30мл"))
    result = vision.recognize(photo(tmp_path), items(settings), settings, None, config, client)
    assert result.ok, result.error
    assert result.candidates and result.candidates[0].row == 12
    assert result.candidates[0].source == "текст"

def test_error_is_reported_not_raised(tmp_path):
    settings = make_settings(tmp_path); config = turn_on(settings); client = FakeClient(error="GigaChat ответил ошибкой 429: лимит")
    result = vision.recognize(photo(tmp_path), items(settings), settings, None, config, client)
    assert not result.ok
    assert "429" in result.error

def test_gigachat_answer_is_parsed():
    assert vision._first_text({"choices": [{"message": {"content": "готово"}}]}) == "готово"
    assert vision._first_text({}) == ""

def test_api_version_is_remembered(tmp_path):
    settings = make_settings(tmp_path); turn_on(settings); vision._remember_scope(settings, "GIGACHAT_API_B2B")
...
def test_human_answers_are_remembered(tmp_path):
    settings = make_settings(tmp_path); turn_on(settings)
    vision.remember(settings, photo="photo.jpg", digest="abc", text="HUSKY MINT", row=12, name="Жидкость Husky Mint 30мл", picked=True, source="модель")

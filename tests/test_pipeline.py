...
GROUPS = {"Сигареты": [("Сигареты Parliament Aqua Blue", -3, -450.0), ("Сигареты Parliament Aqua Blue КС", 3, 450.0), ("Сигареты Winston XS Blue", -2, -300.0), ("Сигареты Kent Nano White", 4, 600.0)], "Стики": [("Стики Heets Amber", -1, -120.0), ("Стики HEETS Amber Selection", 1, 120.0)]}
...
def _build_source(path: Path) -> None:
    workbook = openpyxl.Workbook(); sheet = workbook.active; sheet.title = "TDSheet"
    sheet["A1"] = "Инвентаризация товаров № ИНВ-15 от 09.09.2026"; sheet["A2"] = "Организация: ООО Торг"; sheet["A3"] = "Склад:"; sheet["H3"] = "Валюта"; sheet["I3"] = "руб"
    row = 5
    for name, items in GROUPS.items():
        sheet.cell(row=row, column=1).value = "№"; sheet.cell(row=row, column=2).value = name; sheet.cell(row=row, column=3).value = "Хар-ка"; sheet.merge_cells(start_row=row, start_column=2, end_row=row + 1, end_column=2); row += 2
        for number, (item_name, diff, sum_diff) in enumerate(items, start=1):
            sheet.cell(row=row, column=1).value = number; sheet.cell(row=row, column=2).value = item_name; sheet.cell(row=row, column=4).value = 10; sheet.cell(row=row, column=5).value = 10 - diff; sheet.cell(row=row, column=6).value = diff; sheet.cell(row=row, column=7).value = 150.0; sheet.cell(row=row, column=10).value = sum_diff; sheet.cell(row=row, column=11).value = f"Документ {number}"; row += 1
        sheet.cell(row=row, column=4).value = len(items) * 10; sheet.cell(row=row, column=6).value = sum(item[1] for item in items); sheet.cell(row=row, column=10).value = sum(item[2] for item in items); row += 2
    workbook.save(path)

def _rewrite(path: Path, change) -> None:
    with zipfile.ZipFile(path) as archive: items = [(item, archive.read(item.filename)) for item in archive.infolist()]
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as archive:
        for item, data in items:
            new_data = change(item.filename, data)
            if new_data is not None: archive.writestr(item, new_data)

def _drop_shared_strings(path: Path) -> None: _rewrite(path, lambda name, data: None if name == "xl/sharedStrings.xml" else data)

def _break_cell_styles(path: Path) -> None:
    def change(name: str, data: bytes) -> bytes:
        return re.sub(rb'<c r="B(\d+)"', rb'<c s="99" r="B\1"', data) if name.startswith("xl/worksheets/") else data
    _rewrite(path, change)

def _break_fonts(path: Path) -> None:
    def change(name: str, data: bytes) -> bytes:
        return re.sub(rb'fontId="\d+"', b'fontId="77"', data) if name == "xl/styles.xml" else data
    _rewrite(path, change)

@pytest.fixture()
def source_file(tmp_path: Path) -> Path:
    path = tmp_path / "ОхтаМоллСМА без форматирования.xlsx"; _build_source(path); _drop_shared_strings(path); _break_cell_styles(path); return path
...
def test_output_filename() -> None: assert output_filename("ОхтаМоллСМА", "09.09.2026") == "ОхтаМоллСМА 09.09.2026.xlsx"

def test_accept_date_counts_from_processing_day() -> None:
    assert accept_date() == _accept_text(); assert accept_date("") == _accept_text(); assert accept_date("30.09.2026") == "03.10.2026"; assert accept_date(date(2026, 9, 9)) == "12.09.2026"

def test_normalize_folds_names() -> None:
    first = normalize("Сигареты Parliament Aqua Blue", ("сигареты", "стики")); second = normalize("Сигареты  Parliament Aqua Blue КС", ("сигареты", "стики")); assert first; assert second.startswith(first)

def test_broken_file_fails_without_repair(source_file: Path) -> None:
    with pytest.raises((IndexError, KeyError, ValueError)): openpyxl.load_workbook(source_file)

def test_repair_fixes_styles_and_strings(source_file: Path, tmp_path: Path) -> None:
    assert needs_repair(source_file) is True; assert describe(source_file)["style_problems"] > 0
    repaired = repair_by_inject(source_file, tmp_path / "repaired.xlsx"); assert needs_repair(repaired) is False
    assert openpyxl.load_workbook(repaired)["TDSheet"]["A1"].value.startswith("Инвентаризация товаров")

def test_repair_fixes_broken_fonts(tmp_path: Path) -> None:
    path = tmp_path / "ОхтаМоллСМА без форматирования.xlsx"; _build_source(path); _break_fonts(path); assert needs_repair(path) is True
    openpyxl.load_workbook(repair_by_inject(path, tmp_path / "repaired-fonts.xlsx"))
